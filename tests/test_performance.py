...
async def test_vad_performance():
    ...
    logger.info(f"VAD处理延迟: {avg_time:.2f}ms/帧")
...
async def test_cache_performance():
    ...
    logger.info(f"缓存写入延迟: {write_avg:.3f}ms/次")
    logger.info(f"缓存读取延迟: {read_avg:.3f}ms/次")
...
async def test_interaction_optimizer_performance():
    ...
    elapsed = (time.time() - start_time) * 1000
    logger.info(f"防抖器检查延迟: {elapsed/iterations:.3f}ms/次")
    ...
    elapsed = (time.time() - start_time) * 1000
    logger.info(f"节流器检查延迟: {elapsed/iterations:.3f}ms/次")
...
async def test_audio_preprocessor_performance():
    ...
    logger.info(f"音频预处理延迟: {avg_time:.2f}ms/秒音频")
...
async def test_speed_controller_performance():
    ...
    logger.info(f"语速计算延迟: {avg_time:.3f}ms/次")
...
async def test_emotion_voice_performance():
    ...
    logger.info(f"情感语音计算延迟: {avg_time:.3f}ms/次")
...
async def test_memory_usage():
    ...
    logger.info(f"内存增长: {peak_memory - initial_memory:.1f}MB")
...
async def test_concurrent_performance():
    ...
    logger.info(f"并发写入延迟: {elapsed:.2f}ms (10任务 x 100次写入)")
...
async def test_startup_optimizer_performance():
    ...
    logger.info(f"启动优化加载延迟: {elapsed:.2f}ms (10个模块)")
