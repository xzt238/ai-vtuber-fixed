...
class TextAudioSpeakerLoaderV3(torch.utils.data.Dataset):
    ...
    def get_audio_text_speaker_pair(self, audiopath_sid_text):
        ...
        try:
            ...
            with torch.no_grad():
                ssl_raw = torch.load("%s/%s.pt" % (self.path4, audiopath), map_location="cpu")
                # 确保 ssl 是 3D [1, 768, T]（TextAudioSpeakerLoaderV3 + Collate 期望）
                # 旧文件可能保存为 [T, 768] (2D)，这里统一转为 [1, 768, T]
                if ssl_raw.dim() == 1:
                    ssl = ssl_raw.unsqueeze(0).unsqueeze(0)       # [T] -> [1, 1, T]
                elif ssl_raw.dim() == 2:
                    ssl = ssl_raw.T.unsqueeze(0)                 # [T, 768] -> [768, T] -> [1, 768, T]
                else:
                    ssl = ssl_raw                                 # 本身就是 [1, 768, T]
                if ssl.shape[-1] != spec.shape[-1]:
                    ssl = F.pad(ssl.float(), (0, 1), mode="replicate").to(ssl.dtype)
                ssl = ssl.requires_grad_(False)
        except:
            traceback.print_exc()
