...
# 获取 GPT-SoVITS 根目录
GPT_SOVITS_DIR = Path(__file__).parent.parent.parent
...
from process_ckpt import get_sovits_version_from_path_fast, load_sovits_new, _torch_load_with_utils_swap
...
class TTS:
    ...
    def init_vits_weights(self, weights_path: str):
        ...
        rel_path = self.configs.default_configs[model_version]["vits_weights_path"]
        path_sovits = str(GPT_SOVITS_DIR / rel_path)

        if if_lora_v3 == True and not os.path.exists(path_sovits):
            info = path_sovits + i18n("SoVITS %s 底模缺失，无法加载相应 LoRA 权重" % model_version)
            raise FileExistsError(info)

        # dict_s2 = torch.load(weights_path, map_location=self.configs.device,weights_only=False)
        dict_s2 = load_sovits_new(weights_path)
        # 兼容两种 checkpoint 格式：自定义格式有 "weight"，Lightning 格式有 "state_dict"
        if "weight" not in dict_s2 and "state_dict" in dict_s2:
            print(f"[INFO] SoVITS: 将 state_dict 转换为 weight 格式")
            dict_s2["weight"] = dict_s2["state_dict"]
        
        # 如果 checkpoint 没有 config 字段，从预训练模型读取
        if "config" not in dict_s2 or dict_s2.get("config") is None:
            print(f"[WARN] SoVITS Checkpoint 缺少 config 字段，从预训练模型读取")
            pretrained_vits_path = str(GPT_SOVITS_DIR / "GPT_SoVITS/pretrained_models/s2Gv3.pth")
            if os.path.exists(pretrained_vits_path):
                pretrained_dict = _torch_load_with_utils_swap(pretrained_vits_path, map_location="cpu", weights_only=False)
                dict_s2["config"] = pretrained_dict["config"]
                print(f"[INFO] 已从 s2Gv3.pth 读取 config")
            else:
                dict_s2["config"] = {
                    "model": {"semantic_frame_rate": "25hz", "inter_channels": 192},
                    "data": {
                        "filter_length": 2048,
                        "sampling_rate": 32000,
                        "hop_length": 640,
        ...
        # ===== SSL层冻结：防止音色不稳定 =====
        # LoRA训练时SSL层被冻结从未更新，但推理时如果某些路径激活SSL输出会导致音色漂移
        # 冻结后SSL层输出恒为预训练均值，避免偶发性音色变化
        if hasattr(vits_model, 'enc_p'):
            if hasattr(vits_model.enc_p, 'ssl_proj'):
                vits_model.enc_p.ssl_proj.requires_grad_(False)
                for p in vits_model.enc_p.ssl_proj.parameters():
                    p.requires_grad_(False)
                print("[TTS] enc_p.ssl_proj frozen")
            if hasattr(vits_model.enc_p, 'encoder_ssl'):
                for p in vits_model.enc_p.encoder_ssl.parameters():
                    p.requires_grad_(False)
                vits_model.enc_p.encoder_ssl.requires_grad_(False)
                print("[TTS] enc_p.encoder_ssl frozen")
        # ===== SSL冻结结束 =====
    ...
    def init_t2s_weights(self, weights_path: str):
        ...
        # 如果 checkpoint 没有 config 字段，使用默认值
        if "config" not in dict_s1 or dict_s1.get("config") is None:
            print(f"[WARN] Checkpoint 缺少 config 字段，使用默认配置")
            # 动态读取音素词表大小，避免扩展词典后写死 512 导致推理越界
            try:
                from text.symbols2 import symbols as _symbols2
                _phoneme_vocab_size = len(_symbols2)
            except Exception:
                _phoneme_vocab_size = 512
            print(f"[INFO] phoneme_vocab_size = {_phoneme_vocab_size}")
            dict_s1["config"] = {
                "data": {"max_sec": 54, "max_eval_sample": 8, "num_workers": 2, "pad_val": 1024},
                "model": {
                    "vocab_size": 1025,
                    "phoneme_vocab_size": _phoneme_vocab_size,
                    "embedding_dim": 512,
                    "hidden_dim": 512,
                    "head": 16,
                    "linear_units": 2048,
        ...
        # 兼容两种 checkpoint 格式：自定义格式有 "weight"，Lightning 格式有 "state_dict"
        if "weight" in dict_s1:
            t2s_model.load_state_dict(dict_s1["weight"])
        elif "state_dict" in dict_s1:
            state_dict = dict_s1["state_dict"]
            # Lightning 保存的 state_dict 没有 "model." 前缀，但模型期望有
            # 需要添加 "model." 前缀来匹配模型结构
            new_state_dict = {}
            ...
            if has_model_prefix:
                ...
                # 没有 model. 前缀，需要添加
                print(f"[INFO] 为 state_dict 添加 'model.' 前缀")
                for key, value in state_dict.items():
                    new_state_dict["model." + key] = value
                t2s_model.load_state_dict(new_state_dict)
        else:
            raise ValueError(f"Checkpoint 格式不支持：缺少 'weight' 或 'state_dict' 字段。Keys: {list(dict_s1.keys())}")
    ...
    def init_vocoder(self, version: str):
        if version == "v3":
            ...
            # 使用 GPT_SOVITS_DIR 绝对路径
            bigvgan_path = str(GPT_SOVITS_DIR / "GPT_SoVITS" / "pretrained_models" / "models--nvidia--bigvgan_v2_24khz_100band_256x")
            print(f"[INFO] Loading BigVGAN from: {bigvgan_path}")
