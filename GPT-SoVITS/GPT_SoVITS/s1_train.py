            ...
            if self._every_n_epochs >= 1 and (trainer.current_epoch + 1) % self._every_n_epochs == 0:
                ...
                if self.if_save_every_weights == True:
                    ...
                    if "data" not in self.config:
                        print(f"[WARN] config 缺少 'data' 字段，使用默认值")
                        self.config["data"] = {"max_sec": 54, "max_eval_sample": 8, "num_workers": 2, "pad_val": 1024}
                    # torch.save(
                    # print(os.environ)
                    if os.environ.get("LOCAL_RANK", "0") == "0":
                        print(f"[DEBUG] 保存 checkpoint，config keys: {list(self.config.keys())}")
                        my_save(
                            to_save_od,
                            "%s/%s-e%s.ckpt"
                            % (
...
def main(args):
    ...
    trainer: Trainer = Trainer(
        ...
        val_check_interval=config["train"].get("val_check_interval", 1.0),
        ...
        callbacks=[ckpt_callback, TQDMProgressBar()],
    ...
    data_module: Text2SemanticDataModule = Text2SemanticDataModule(
        ...
        dev_semantic_path=config.get("dev_semantic_path"),
        dev_phoneme_path=config.get("dev_phoneme_path")
    )
