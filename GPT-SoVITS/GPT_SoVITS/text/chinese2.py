...
if is_g2pw:
    ...
    # BERT 模型路径通过环境变量传入（由 gptsovits.py 设置）
    # 环境变量由 gptsovits.py 在导入前设置
    bert_path = os.environ.get("GPT_SOVITS_BERT_PATH", "")
    if not bert_path:
        # Fallback: 使用 file 位置向上查找
        parent_directory = os.path.dirname(current_file_path)  # GPT_SoVITS/GPT_SoVITS/text
    ...
    g2pw_model_dir = os.path.join(os.path.dirname(current_file_path), "G2PWModel")
    g2pw = G2PWPinyin(
