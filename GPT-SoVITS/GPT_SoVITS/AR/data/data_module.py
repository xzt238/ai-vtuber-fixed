...
class Text2SemanticDataModule(LightningDataModule):
    ...
    def setup(self, stage=None, output_logs=False):
        ...
        # 如果提供了 dev 路径，使用独立验证集；否则复用训练集
        if self.dev_semantic_path and self.dev_phoneme_path:
            ...
            print(f"[DATA] 使用独立验证集: {self.dev_phoneme_path}")
            print(f"[DATA] 训练集: {len(self._train_dataset)} 样本, 验证集: {len(self._dev_dataset)} 样本")
        else:
            self._dev_dataset = self._train_dataset
            print(f"[DATA] 未提供验证集，使用训练集作为验证集")
