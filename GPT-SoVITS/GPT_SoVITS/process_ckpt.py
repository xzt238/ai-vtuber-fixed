...
def _torch_load_with_utils_swap(obj, **kwargs):
    """Wrap torch.load with proper utils module swap for GPT-SoVITS checkpoints.
    ai-vtuber app may have its own 'utils' module that shadows GPT_SoVITS/utils,
    causing pickle to fail with 'Can't get attribute HParams'."""
    _orig_utils = sys.modules.get("utils")
    try:
        # Must delete sys.modules['utils'] first, otherwise importlib.find_module
        # (used by __import__) returns the shadowing module instead of re-finding it
        if "utils" in sys.modules:
            del sys.modules["utils"]
        import importlib
        _gpt_utils = importlib.import_module("utils")
        sys.modules["utils"] = _gpt_utils
        return torch.load(obj, **kwargs)
    finally:
...
def load_sovits_new(sovits_path):
    ...
    if meta == b"PK":
        # 标准 ZIP 格式，直接用路径加载
        load_obj = sovits_path
    else:
        # 版本标签格式：前2字节是 "03"，重建 ZIP
        bio = BytesIO(b"PK" + data)
        load_obj = bio

    return _torch_load_with_utils_swap(load_obj, map_location="cpu", weights_only=False)
