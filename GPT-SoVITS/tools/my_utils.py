...
def load_audio(file, sr):
    ...
    # WAV 文件直接用 soundfile 读取（无需 ffmpeg）
    ext = os.path.splitext(file)[1].lower()
    if ext in (".wav", ".flac", ".ogg"):
        try:
            audio_data, orig_sr = sf.read(file, dtype="float32")
            # 单声道转多声道：直接复制到 num_frames
            if len(audio_data.shape) == 1:
                audio_data = np.expand_dims(audio_data, axis=1)
            # 归一化到 [-1, 1]（soundfile 读出的 WAV 通常已归一化）
            audio_data = audio_data.flatten()
            # 重采样（如果需要）
            if sr != orig_sr:
                audio_data = resample(audio_data, orig_sr, sr)
            return audio_data
        except Exception as e:
            print(f"[load_audio] soundfile 读取失败 ({file}): {e}，尝试 ffmpeg...")

    # 非 WAV 格式或其他情况：用 ffmpeg
    try:
        out, _ = (
