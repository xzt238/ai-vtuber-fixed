...
| 功能 | 说明 | 状态 |
|------|------|------|
| 🎭 **Live2D V3** | 粒子+物理+情感引擎 | ✅ |
| 🧊 **VRM 3D** | Three.js 渲染 | ✅ |
| 🤖 **12 种 LLM** | OpenAI/Claude/通义千问等 | ✅ |
| 🎤 **15 种 TTS** | 情感语音+口型同步 | ✅ |
| 🗣️ **7 种 ASR** | Whisper/百度/讯飞等 | ✅ |
| 📚 **RAG 知识库** | 向量检索增强 | ✅ |
| 👥 **多角色群聊** | 4 种预设场景+自定义 | ✅ |
| 📞 **语音通话** | 实时语音对话 | ✅ |
| 🤖 **多 Agent** | 5 种协作模式 | ✅ |
...
| 操作 | 命令 | 说明 |
|------|------|------|
| **构建 APK** | `build.bat android` | 生成 Android 安装包 |
| **构建 iOS** | `build.bat ios` | 生成 iOS 安装包（需要 Apple 开发者账号） |
| **OTA 热更新** | `update.bat` | 推送 JS 代码更新，用户打开 App 自动下载 |
| **重新构建** | `rebuild.bat` | 大版本更新，需要重新安装 |
...
1. **共享配置** - `shared/config.yaml` 定义统一配置格式
2. **功能对齐** - 新功能同时在桌面端和移动端实现
3. **版本同步** - 两端版本号保持一致
4. **代码复用** - 核心逻辑在两端分别实现，保持接口一致

**开发流程**：
```
新功能开发 → 桌面端实现 → 移动端实现 → 同步测试 → 发布
```
