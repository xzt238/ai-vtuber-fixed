...
### v1.9.39 (2026-04-30) — LLM Provider 自动选择 + 后端引擎热重建

**新增** 切换 Provider 自动填入默认模型；**修复** 后端不重建 LLM 引擎的问题

- ✨ `onLlmProviderChange()`: 切换 Provider 自动填入默认模型（MiniMax→MiniMax-M2.7, OpenAI→qwen3:8b, Anthropic→claude-3-sonnet）
- ✨ `getDefaultModel(provider)`: 统一获取默认模型名
- ✨ 新手引导也自动填写默认模型名
- 🔧 后端 `_handle_config()`: Provider 变更时自动重建 LLM 引擎（清理缓存 + 下次访问用新配置创建）
- 🔧 切换 Provider 后自动重置会话历史、同步 API Key
